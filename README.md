...
- **两种查询模式**（数据同源，均来自 `/api/log/self` 原始日志，含**令牌名称**）：
  - 按日查询：将日志按「日期 + 令牌名称 + 模型」聚合，展示调用次数、提示/补全 Tokens、花费（可用于出账）。
  - 按条查询：展示逐条调用明细（时间、令牌、模型、用时、提示/补全、花费、计费详情），**不做条数截断**。

> 取数说明：NewAPI `/api/log/self` 的 `page_size` 硬上限为 100，且深度分页（如 `OFFSET 244400`）会触发慢 SQL、抬高数据库 load。本项目按时间范围**按天切窗 + 有限并发（默认 4）**拉取，使每个查询的 `OFFSET` 只在单天行数内，避免深度 OFFSET 慢查询；**结果与不切窗完全一致（全量）**。切换「按日/按条」仅切换展示，不会重新请求。
- **模型筛选** + 「重置筛选」。
