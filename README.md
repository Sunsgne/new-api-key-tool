...
- **两种查询模式**：
  - 按日查询：在「访问令牌」方式下调用聚合接口 `/api/data/self` 得到按「日期 + 模型」的汇总（调用次数、消耗 Tokens、花费）。该接口服务端限制单次时间跨度 ≤1 个月，本项目会**按月自动分片**后合并，支持任意范围（整月/季度/年出账）；若服务端未开启「数据看板」（聚合为空），自动回退为逐日 `sum(quota)` 精确求和（金额准确、可用于出账，但不含次数/Tokens 明细）。
  - 按条查询：调用 `/api/log/self` 拉取逐条明细，采用**并发分页**（大页 + 多并发）显著加速。

> 性能说明：日志量很大（数万条）时，「按条查询」需要翻很多页，建议优先使用「按日查询」（聚合接口，单请求返回）。切换查询模式会按对应接口自动重新查询。
>
> 关于「按条查询」的后台压力：NewAPI `page_size` 硬上限为 100，且深度分页（如 `OFFSET 244400`）会触发慢 SQL、抬高数据库 load。本项目对「按条查询」做了**按天切窗 + 有限并发（默认 3）**，使每个查询的 `OFFSET` 只在单天行数内，避免深度 OFFSET 慢查询；并通过 `REACT_APP_MAX_DETAIL_ROWS`（默认 50000）限制单次明细总量。**超大范围请始终用「按日查询」。**
- **模型筛选** + 「重置筛选」。
